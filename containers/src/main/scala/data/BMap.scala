...
sealed trait BMap[K, A] {
  ...
  def insertWith(fx: (A, A) => A, kx: K, x: A)(implicit o: Order[K]): BMap[K, A] = insertWithKey((_, a, b) => fx(a, b), kx, x)(o)
  ...
  def adjust(v: A => A, k: K)(implicit o: Order[K]): BMap[K, A] = adjustWithKey((_, a) => v(a))(k)
    ...
    def go(m: BMap[K, A]): (Option[A], BMap[K, A]) = m match {
      ...
      case Bin(sx, kx, x, l, r) => o.order(k, kx) match {
        ...
        case EQ => f(kx, x) match {
          ...
          case Some(a) => (Some(a), Bin(sx, kx, a, l, r))
        }
      }
    }
    go(this)
